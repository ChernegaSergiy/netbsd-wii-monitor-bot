 ...
 * @return SQLite3
 */
function initDatabase(string $db_file) : SQLite3
{
 ...
 * @return array|false
 */
function sendTelegramMessage(string $bot_token, int $chat_id, string $text, ?array $keyboard = null) : array|false
{
 ...
 * @return array|false
 */
function editTelegramMessage(string $bot_token, int $chat_id, int $message_id, string $text, ?array $keyboard = null) : array|false
{
 ...
 * @return bool
 */
function isAdmin(int $user_id, array $admin_ids) : bool
{
 ...
 * @return array|false
 */
function getCombinedData(string $url, SQLite3 $db, int $max_retries = 3) : array|false
{
 ...
 * @return string
 */
function convertTimezone(string $timestamp_str, string $from_tz, string $to_tz) : string
{
    try {
 ...
 * @return bool
 */
function isRecent(string $timestamp_str, int $minutes, SQLite3 $db) : bool
{
 ...
 * @return string|false
 */
function takeScreenshot(string $target_url, SQLite3 $db, int $max_retries = 3) : string|false
{
    for ($attempt = 1; $attempt <= $max_retries; $attempt++) {
        $puppeteer_server = getSetting($db, 'puppeteer_server');
        $viewport_width = getSetting($db, 'viewport_width');
 ...
 * @return bool
 */
function sendScreenshot(string $image_path, string $caption, SQLite3 $db) : bool
{
    global $bot_token;
 ...
 * @return bool
 */
function isScreenshotComplete(string $content) : bool
{
 ...
 * @return string
 */
function testCheck(SQLite3 $db, int $chat_id) : string
{
 ...
 * @return string
 */
function _performFullTestAndReturnResult(SQLite3 $db) : string
{
    global $messages;
 ...
 * @return void
 */
function processUpdate(array $update, string $bot_token, array $admin_ids, SQLite3 $db) : void
{
 ...
 * @return array
 */
function getUpdates(string $bot_token, int $offset) : array
{
